...
function S(e) {
  ...
  (function (e, t) {
    if (!b.has(e)) {
      b.set(e, t);
      if (!R) {
        // MOVED TO DOCUMENT LEVEL FOR Z-INDEX HANDLING
        document.addEventListener('pointermove', M);
        document.addEventListener('pointerleave', L);
  ...
  t.dispose = () => {
    ...
    if (b.size === 0) {
      ...
      document.removeEventListener('pointerleave', L);
...
function processInteraction() {
  for (const [elem, t] of b) {
    // If the element provides a domElement that it considers its bounding box use it, otherwise use window
    const i = elem.getBoundingClientRect(); 
    // Usually the canvas is full screen anyway.
    if (D(i)) {
      P(t, i);
      if (!t.hover) {
        t.hover = true;
...
class Z extends d {
  ...
  setColors(e) {
    if (Array.isArray(e) && e.length > 0) {
      const t = e.map(col => new l(col));
      for (let idx = 0; idx < this.count; idx++) {
        const colorIndex = idx % t.length;
        this.setColorAt(idx, t[colorIndex]);
        if (idx === 0) {
          this.light.color.copy(t[colorIndex]);
        }
      }
...
const BallpitEffect = ({ className = '', count = 100, gravity = 0.01, friction = 0.9975, wallBounce = 0.95, followCursor = true, colors = ['#ffffff', '#cccccc', '#6b21a8', '#111827'], ...props }) => {
  const canvasRef = useRef(null);
  const spheresInstanceRef = useRef(null);
  const lastPropsRef = useRef({ count, gravity, friction, wallBounce, followCursor });
  ...
  useEffect(() => {
    ...
    spheresInstanceRef.current = createBallpit(canvas, { count, gravity, friction, wallBounce, followCursor, colors, ...props });
  ...
  useEffect(() => {
    ...
    if (spheresInstanceRef.current && spheresInstanceRef.current.spheres) {
        ...
        spheres.config.friction = friction;
        spheres.config.wallBounce = wallBounce;
        spheres.config.followCursor = followCursor;
        spheres.config.colors = colors;
        
        if (count !== lastPropsRef.current.count) {
           spheresInstanceRef.current.setCount(count);
           // Re-apply colors after count updates as it regenerates instances
           spheresInstanceRef.current.spheres.setColors(colors);
        } else {
           spheres.setColors(colors);
        }
        lastPropsRef.current = { count, gravity, friction, wallBounce, followCursor };
    }
  }, [gravity, friction, wallBounce, followCursor, count, colorsStr]);
